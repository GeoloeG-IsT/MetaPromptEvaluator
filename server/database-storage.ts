...
export class DatabaseStorage implements IStorage {
  ...
  async updatePrompt(
    ...
    if (prompt.metaPrompt !== undefined)
      updateValues.metaPrompt = prompt.metaPrompt;
  ...
  async deletePrompt(id: number): Promise<boolean> {
    ...
    const promptEvaluations = await db
      .select()
      .from(evaluations)
      .where(eq(evaluations.promptId, id));

    // Delete all evaluation results for each evaluation
    for (const evaluation of promptEvaluations) {
      await db
        .delete(evaluationResults)
        .where(eq(evaluationResults.evaluationId, evaluation.id));
    }
  ...
  async updateEvaluation(
    ...
    if (evaluation.status !== undefined)
      updateValues.status = evaluation.status;
    if (evaluation.score !== undefined) updateValues.score = evaluation.score;
    if (evaluation.metrics !== undefined)
      updateValues.metrics = evaluation.metrics;
    if (evaluation.completedAt !== undefined)
      updateValues.completedAt = evaluation.completedAt;
    if (evaluation.promptId !== undefined)
      updateValues.promptId = evaluation.promptId;
    if (evaluation.datasetId !== undefined)
      updateValues.datasetId = evaluation.datasetId;
    if (evaluation.userPrompt !== undefined)
      updateValues.userPrompt = evaluation.userPrompt;
    if (evaluation.finalPrompt !== undefined)
      updateValues.finalPrompt = evaluation.finalPrompt;

    // Log the actual updateValues being used
    console.log(
      `Database updateEvaluation for ID ${id} with fields:`,
  ...
  async getDashboardStats(): Promise<DashboardStats> {
    // Total prompts count
    const promptsCountResult = await db
      .select({ count: sql<number>`count(*)` })
      .from(prompts);
    const totalPrompts = promptsCountResult[0]?.count || 0;

    // Total evaluations count
    const evalsCountResult = await db
      .select({ count: sql<number>`count(*)` })
      .from(evaluations);
    ...
    const dataElementsResult = await db
      .select({ count: sql<number>`count(*)` })
      .from(datasetItems);
