...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/prompts", async (req: Request, res: Response) => {
    try {
      ...
      if (error instanceof z.ZodError) {
        res
          .status(400)
          .json({ message: "Invalid prompt data", errors: error.format() });
      } else {
  ...
  app.post("/api/datasets", async (req: Request, res: Response) => {
    try {
      ...
      if (error instanceof z.ZodError) {
        res
          .status(400)
          .json({ message: "Invalid dataset data", errors: error.format() });
      } else {
    ...
    try {
      ...
      for (const item of datasetItems) {
        ...
        if (item.inputType === "pdf" && item.inputPdf) {
          try {
            console.log(
              `Deleting PDF file for item ${item.id}: ${item.inputPdf}`,
            );
  ...
  app.post("/api/dataset-items", async (req: Request, res: Response) => {
    try {
      ...
      if (error instanceof z.ZodError) {
        console.error(
          "POST /api/dataset-items - Validation error:",
          error.format(),
        );
        res.status(400).json({
          message: "Invalid dataset item data",
          errors: error.format(),
        });
      } else {
  ...
  app.get("/api/datasets/:id/items", async (req: Request, res: Response) => {
    try {
      const datasetId = parseInt(req.params.id);
      console.log(
        `GET /api/datasets/${datasetId}/items - Fetching items for dataset ID: ${datasetId}`,
      );
      const datasetItems = await storage.getDatasetItems(datasetId);
      console.log(
        `GET /api/datasets/${datasetId}/items - Found ${datasetItems.length} items`,
      );
  ...
  app.put("/api/dataset-items/:id", async (req: Request, res: Response) => {
    try {
      ...
      if (existingItem.inputType === "pdf" && existingItem.inputPdf) {
        ...
        const isInputTypeChanging =
          req.body.inputType && req.body.inputType !== "pdf";
        const isPdfFileChanging =
          req.body.inputPdf && req.body.inputPdf !== existingItem.inputPdf;

        if (isInputTypeChanging || isPdfFileChanging) {
          try {
            console.log(
              `Deleting old PDF file for updated item ${id}: ${existingItem.inputPdf}`,
            );
            await bucketStorage.deletePdf(existingItem.inputPdf);
            console.log("Old PDF and markdown files deleted successfully");
          } catch (pdfError) {
            console.error(
              "Error deleting old PDF file during update:",
              pdfError,
            );
  ...
  app.post("/api/pdf-upload", async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ message: `Failed to upload PDF file: ${error.message}` });
    }
  });
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!processedPrompt) {
          return res
            .status(400)
            .json({ message: "Processed prompt is required" });
        }
  ...
  app.post("/api/evaluations", async (req: Request, res: Response) => {
    try {
      ...
      if (error instanceof z.ZodError) {
        res
          .status(400)
          .json({ message: "Invalid evaluation data", errors: error.format() });
      } else {
  ...
  app.put("/api/evaluations/:id", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`PUT /api/evaluations/${id} body:`, {
        promptId,
        datasetId,
        userPrompt,
      });
      ...
      if (evaluationResults.length > 0) {
        ...
        console.log(
          `Deleted ${evaluationResults.length} evaluation results for evaluation ${id}`,
        );
      }
      ...
      if (promptId !== undefined || userPrompt !== undefined) {
        ...
        const targetPromptId =
          promptId !== undefined ? promptId : existingEvaluation.promptId;
        const targetUserPrompt =
          userPrompt !== undefined ? userPrompt : existingEvaluation.userPrompt;
        ...
        if (prompt) {
          console.log(
            "Using meta prompt:",
            prompt.metaPrompt?.substring(0, 50) + "...",
          );
          ...
          try {
            ...
            console.log(
              "Generated final prompt for update:",
              finalPrompt.substring(0, 100) + "...",
            );
            updateData.finalPrompt = finalPrompt;
          } catch (promptError) {
            console.error(
              "Error generating final prompt for update:",
              promptError,
            );
            ...
            const finalPrompt = targetUserPrompt
              ...
              : prompt.metaPrompt;
            console.log(
              "Falling back to simple replacement for final prompt in update",
            );
            updateData.finalPrompt = finalPrompt;
          }
        } else {
        ...
        console.log(
          "No prompt ID or user prompt changes, skipping final prompt generation",
        );
      }
      ...
      console.log(`Final updateData being saved to database:`, {
        ...updateData,
        finalPrompt: updateData.finalPrompt || null,
      });
      ...
      console.log(`Updated evaluation returned from database:`, {
        ...
        finalPrompt: updatedEvaluation?.finalPrompt || null,
      });
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (evaluation.status === "in_progress") {
          return res
            .status(400)
            .json({ message: "Evaluation is already in progress" });
        }
        ...
        if (userPrompt !== undefined) {
          ...
          if (prompt) {
            try {
              ...
              console.log(
                "Generated new final prompt for evaluation start:",
                finalPrompt
              );
              ...
              console.error(
                "Error generating final prompt during evaluation start:",
                promptError,
              );
              // Fall back to simple replacement
              const finalPrompt = prompt.metaPrompt.replace(
                /{{user_prompt}}/g,
                userPrompt,
              );
        ...
        setTimeout(async () => {
          try {
            ...
            if (!updatedEvaluation) {
              console.error(
                `Evaluation with ID ${evaluationId} not found during processing`,
              );
              return;
            }

            const prompt = await storage.getPrompt(updatedEvaluation.promptId);
            const datasetItems = await storage.getDatasetItems(
            ...
            if (updatedEvaluation.finalPrompt) {
              ...
              try {
                ...
                console.error(
                  "Error generating final prompt during evaluation:",
                  promptError,
                );
                // Fall back to simple replacement
